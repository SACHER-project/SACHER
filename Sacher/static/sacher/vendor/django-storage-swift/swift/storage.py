...
class SwiftStorage(Storage):
    ...
    object_storage_url = setting('SWIFT_OBJECT_STORAGE_URL')
        ...
        data = {
            "auth": {
                "identity": {
                    ...
                    "password": {
                        "user": {
                            "name": str(self.api_username),
                            "domain": {
                                "id": str(self.user_domain_id),
                            },
                            "password": str(self.api_key),
                        }
                    }
                },
                "scope": {
                    "project": {
                        "name": str(self.tenant_name),
                        "domain": {
                            "id": str(self.project_domain_id),
                        }
                    }
                }
            }
        }
        req = urllib2.Request(str(self.api_auth_url) + '/auth/tokens')
        req.add_header('Content-Type', 'application/json')
        response = urllib2.urlopen(req, json.dumps(data))
        self._token = str(response.headers['x-subject-token'])
    ...
    def swift_conn(self):
        ...
        if not self._swift_conn:
            ...
            self._swift_conn = swiftclient.Connection(
                preauthtoken=self._token,
                auth_version=3,
                os_options={'object_storage_url': self.object_storage_url, })
